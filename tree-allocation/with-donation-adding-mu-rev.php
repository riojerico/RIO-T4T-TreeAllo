...
$no_t4tlahan=mysql_fetch_array(mysql_query("select no_t4tlahan,koordinat from current_tree where hidup='1' and used='0' and bl='' and no_shipment='' and koordinat!='' and id_pohon='$id_pohon[0]' and kd_mu='$mu[0]'"));
$no=$no_t4tlahan[0];
...
$query_htc=mysql_query("insert into t4t_htc values ('','$add_bl','$destination','$kd_lahan','$no_lahan','$geo','$silvilkultur[0]','$luas','$petani[0]',
                        '$desa[0]','$ta[0]','$add_nama_mu','$treeperwins','','$id_partisipan[0]$date$ns_htc2')");
}
...
for ($i=1; $i <= 1 ; $i++) { 
   //no shipment
    ...
    $ns2=$ns[0]+$i;
    //echo $ns2;  
   $query_current_tree_update=mysql_query("update current_tree set used='1',bl='$add_bl',no_shipment='$id_partisipan[0]$date$ns2' where used='0' and hidup='1' and id_pohon='$id_pohon[0]' and kd_mu='$mu[0]' and no_t4tlahan='$land' limit $treeperwins");
}
